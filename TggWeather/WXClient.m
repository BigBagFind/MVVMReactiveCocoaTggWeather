    ...
    return [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        NSURLSessionDataTask *dataTask = [self.session dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (! error) {
                ...
                if (! jsonError) {
                    // 1当JSON数据存在并且没有错误，发送给订阅者序列化后的JSON数组或字典。
                    [subscriber sendNext:json];
                } else {
                    // 2在任一情况下如果有一个错误，通知订阅者。
            ...
            // 3无论该请求成功还是失败，通知订阅者请求已经完成。
            [subscriber sendCompleted];
        }];
        // 3一旦订阅了信号，启动网络请求。
        [dataTask resume];
        // 4创建并返回RACDisposable对象，它处理当信号摧毁时的清理工作。
        return [RACDisposable disposableWithBlock:^{
            [dataTask cancel];
        }];
    }]doError:^(NSError *error) {
        // 5增加了一个“side effect”，以记录发生的任何错误。side effect不订阅信号，相反，他们返回被连接到方法链的信号。你只需添加一个side effect来记录错误。
        NSLog(@"%@",error);
    }];
...
// http://api.openweathermap.org/data/2.5/weather?lat=35&lon=139&units=imperial&APPID=aaa75dead2f8a91d8d7c5b03ee1c4e04
// api.openweathermap.org/data/2.5/weather?lat=35&lon=139&APPID=aaa75dead2f8a91d8d7c5b03ee1c4e04
// api.openweathermap.org/data/2.5/forecast/city?id=524901&APPID=aaa75dead2f8a91d8d7c5b03ee1c4e04
//   aaa75dead2f8a91d8d7c5b03ee1c4e04
